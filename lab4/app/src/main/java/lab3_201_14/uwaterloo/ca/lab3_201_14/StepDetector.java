        ...
        switch(currentState) {
            ...
            case stateEnd: // Detect return to resting range
                ...
                if (peakToPeak > accelThresholdMin && peakToPeak < accelThresholdMax && y > rangeInitialMin && y < rangeInitialMax) {
                    ...
                    float newX = (float) (MainActivity.origin.x + DisplacementManager.getRoomX());
                    float newY = (float) (MainActivity.origin.y + DisplacementManager.getRoomY());
                    MainActivity.origin.set(newX, newY);
                    PointF tempP = new PointF(newX, newY);
                    positionListener.originChanged(MainActivity.mv, tempP);
                    this.currentState = stateInitial;
